...
# def mamadmeti_tree(students_file_address, ta_python, ta_cpp): list of files, func start do this work
#     '''creats the hw folder'''
...
def start(s, l, cppfolder, pythonfolder):
    aphwpath = os.path.abspath(s)
    hwpath = Path(str(Path(os.path.abspath(aphwpath)).parent) + '\hw')
    # print(hwpath)
    if os.path.exists(hwpath):
        return 'pls remove or rename hw folder'
    else:
        create_tree(s, l, cppfolder, pythonfolder)
        return True

#____________________________________________________________________________

def all_students_grading(root):
    '''runs run_student() in all the folder in the root'''

#____________________________________________________________________________

class student():
    def __init__(self, setting_Data, folder_Address):
        ...
        self.grade_list = {}             #key: number of test, value: point loss of that test. (?!)
    ...
    def pre_process(self):
        ...
        self.report_name_format = 0      
        self.folder_name_format = 0
...
if __name__ == "__main__":
    ...
    print(start(r'E:\APHW1', l, folder, folder2))
